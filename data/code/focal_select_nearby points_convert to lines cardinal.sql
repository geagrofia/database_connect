-- example: select COVERAGE.CONNECT_FOUR_NEIGHBOURS_CARDINAL_NODUPES(4)

CREATE OR REPLACE FUNCTION COVERAGE.CONNECT_FOUR_NEIGHBOURS_CARDINAL_NODUPES(_neighbours NUMERIC)
RETURNS INTEGER AS
$$ -- dollar quotes
DECLARE
...
DROP TABLE IF EXISTS  COVERAGE.FRICTION_B_LINES_CARDINAL_NO_DUPES CASCADE;

CREATE TABLE COVERAGE.FRICTION_B_LINES_CARDINAL_NO_DUPES (
  Id BIGSERIAL,
  Source_Id BIGINT,
  Target_Id BIGINT,
  ...
  CONSTRAINT FRICTION_B_LINES_CARDINAL_NO_DUPES_pk PRIMARY KEY(Id) 
);

DROP INDEX IF EXISTS gist_friction_geom;
CREATE INDEX IF NOT EXISTS gist_friction_geom ON COVERAGE.FRICTION_B_LINES_CARDINAL_NO_DUPES USING GIST (Geom);



-- notify
RAISE NOTICE 'CONNECT_FOUR_CARDINAL_NEIGHBOURS: begun.';
...
INSERT INTO COVERAGE.FRICTION_B_LINES_CARDINAL_NO_DUPES (Source_Id, Target_Id, Source_fric, Target_fric, Distance, Cost,	Geom)
...
-- notify
RAISE NOTICE 'CONNECT_FOUR_CARDINAL_NEIGHBOURS: completed successfully.';
RETURN 0;
